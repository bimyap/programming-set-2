...
def vigenere_cipher(message, key):
    ...
    for i in range(len(msg_list)):
        if msg_list[i] == " ":
            ...
            encrypted_list.append(chr(base + ((msg_list[i] - base + key_list[i]) % 26)))
...
def scytale_cipher(message, shift):
    ...
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    pass
    # Pad message with underscores to make its length a multiple of shift
    ...
    # Rearranging characters column-wise
    for i in range(rows):
